...
export default function FindAKitchen() {
  ...
  // Clicking a card or a map pin keeps the two views in sync.
      ...
      <section className="bg-primary px-4 pb-16 pt-32 text-secondary lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <form
            ...
            className="mx-auto mt-6 flex max-w-xl items-center gap-2 rounded-full bg-white p-2 shadow-xl"
          >
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_1.05fr]">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
              {filtered.map((k, i) => (
                <Link
                  key={k.id}
                  ref={(el) => (cardRefs.current[k.id] = el)}
                  to={`/kitchen/${k.slug}`}
                  onMouseEnter={() => setSelected(k.id)}
                  className={cn(
                    'group flex flex-col overflow-hidden rounded-2xl border bg-card shadow-card transition-all hover:-translate-y-1 hover:shadow-card-hover',
                    selected === k.id && 'ring-2 ring-accent',
                  )}
                >
            ...
            <div className="hidden lg:block">
              <div className="sticky top-24 h-[calc(100vh-8rem)] min-h-[28rem]">
                <KitchenMap kitchens={filtered} selectedId={selected} onSelect={handleSelect} className="h-full border shadow-card" />
              </div>
            </div>
          </div>
        )}
